...
def action_to_id(a):
    ...
    """
    if np.all(a == [-1.0, 0.0, 0.0]): return LEFT               # LEFT: 1
    elif np.all(a == [1.0, 0.0, 0.0]): return RIGHT             # RIGHT: 2
    elif np.all(a == [0.0, 1.0, 0.0]): return ACCELERATE        # ACCELERATE: 3
    elif np.all(a == [0.0, 0.0, 0.2]): return BRAKE             # BRAKE: 4
    else:       
